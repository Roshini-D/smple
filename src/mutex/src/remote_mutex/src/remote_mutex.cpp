...
RemoteMutex::RemoteMutex(std::string name) {
	...
	_client = _nh.serviceClient<remote_mutex_msgs::mutexSrv>(_remoteMutexName,true);
	
}
/*void sethandler(ros::NodeHandle n)
{
	_nh.
}*/
...
bool RemoteMutex::getStatus() {
	...
	//printf("In Get Status\n");
	//printf("%s \n",srv.request.requestor.c_str());
	if (_client.call(srv)) {
		 //printf("Success\n");
